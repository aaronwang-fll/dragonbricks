...
export function useParser() {
  ...
  const processLlmCommands = useCallback(
    async (
          ...
          .map(async (cmd) => {
            try {
              ...
              if (response.success && response.python_code) {
                ...
                updateCommand(cmd.id, {
                  status: 'error',
                  error: response.error || 'Could not understand this command. Try rephrasing it.',
                });
              }
            } catch (error) {
              updateCommand(cmd.id, {
                ...
                error:
                  error instanceof Error
                    ? error.message
                    : 'Could not process this command. Please try again.',
              });
            }
          });
  ...
  const parseInput = useCallback(
    async (input: string): Promise<ParsedCommand[]> => {
      ...
      try {
        ...
        // Always send ALL lines so the backend generates the full program
        // correctly (it joins all parsed commands into one program).
        const response = await api.parseCommands(lines, config, routines);

        // Build full command list from response
        ...
        const commands: ParsedCommand[] = response.results.map((result, index) => {
          ...
          const status: ParsedCommand['status'] =
  ...
  const parseInputSync = useCallback(
  ...
  const resolveClarification = useCallback(
    async (commandId: string, field: string, value: string) => {
