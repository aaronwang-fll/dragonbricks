...
function PortSelect({ value, onChange, allowNone = false }: { value: string; onChange: (v: string) => void; allowNone?: boolean }) {
  return (
    <select
      value={value}
...
function NameablePort({
  ...
  const port = config[portKey as keyof SetupConfig];
  const name = config[nameKey as keyof SetupConfig];
    ...
    <div className="p-3 bg-white dark:bg-gray-800 h-full overflow-y-auto">
      ...
      <div className="mb-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex items-center justify-between p-2 bg-gray-100 dark:bg-gray-700 rounded">
            <span className="text-xs font-medium text-gray-700 dark:text-gray-200">Right Motor</span>
      ...
      <div className="mb-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <NameablePort portKey="attachment1" nameKey="attachment1Name" config={config} editingName={editingName} updateConfig={updateConfig} setEditingName={setEditingName} />
          <NameablePort portKey="attachment2" nameKey="attachment2Name" config={config} editingName={editingName} updateConfig={updateConfig} setEditingName={setEditingName} />
        </div>
      </div>
