          ...
          return (
            <div key={index} className="border-b border-gray-800">
              ...
              <div className="flex items-center">
                ...
                {/* Expand/collapse arrow - only show if there's code */}
                {hasCode && (
                  <button
                    onClick={() => toggleLine(index)}
                    className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-white hover:bg-gray-700 transition-colors flex-shrink-0"
                ...
                {hasCode && isExpanded && (
                  <div className="flex-1 text-xs font-mono text-green-400 truncate px-2 py-2 bg-gray-800/50 rounded-r min-w-0">
                    {pythonCode.split('\n').map((codeLine, i) => (
                      <span key={i} className={
                        codeLine.startsWith('#') ? 'text-gray-500' :
                        codeLine.includes('=') ? 'text-blue-400' :
                        'text-green-400'
                      }>
