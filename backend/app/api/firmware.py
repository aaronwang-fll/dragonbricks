...
async def get_firmware_info():
    ...
    async with httpx.AsyncClient() as client:
        ...
        return {
            ...
            "assets": [
                {
                    "hub_type": asset["name"].split("-")[
                        1
                    ],  # e.g., "pybricks-primehub-v3.6.1.zip" -> "primehub"
                    "name": asset["name"],
                    "size": asset["size"],
...
async def download_firmware(hub_type: str):
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        if not asset:
            raise HTTPException(
                status_code=404, detail=f"No firmware found for hub type: {hub_type}"
            )
        ...
        async def stream_firmware():
            async with httpx.AsyncClient(follow_redirects=True) as download_client:
                async with download_client.stream("GET", download_url) as response:
                    if response.status_code != 200:
                        raise HTTPException(
                            status_code=502, detail="Failed to download firmware from GitHub"
                        )
